...
namespace Engine
{
	...
	bool EpollService::dispatch(epoll_event* events, std::map<fd_t, int>& event_map)
	{
		int ret = 0;
		int events_num = 0;
	...
	void EpollService::event_read(Session* session)
	{
		...
		int recv_size = session->get_socket()->get_readable_size(fd);
		...
		for (; left_size >= 0;)
		{
			...
			if (ret < 0)
			{
				...
				int err = session->get_socket()->get_socket_err(fd);
